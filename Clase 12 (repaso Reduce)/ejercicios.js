const movies = [
  ...
  */
  ...
  /*1. Obtener una lista de los géneros sin repetir */
  const getGenere = (array) =>{
    
    let genrList = array.reduce((acumm, element) => {

        if(!acumm.includes(element.genre)){
        acumm = [...acumm, element.genre]
        }
        return acumm
  ...
  const gerDirectors = (array) =>{
    
    let genrList = array.reduce((acumm, element) => {

        if(!acumm.includes(element.director)){
        acumm = [...acumm, element.director]
        }
        return acumm
  ...
  console.log(gerDirectors(movies))
  
/* 3. Obtener una lista con únicamente los nombres de los actores protagonistas de todas las películas, sin repetir */

 const gerprotagonists = (array) =>{
    
    let genrList = array.reduce((acumm, element) => {

        if(!acumm.includes(element.protagonists[0].name)){
        acumm = [...acumm, element.protagonists[0].name]
        }
        return acumm
  ...
  console.log(gerprotagonists(movies))
 ...
 */
  ...
  const getrating = (array) =>{
    ...
    let ratingList = array.reduce((acumm, element) => {

        if(!acumm.includes(element.rating)){
        acumm = [...acumm, element.rating]
        }
        return acumm
  ...
  const getDuration = (array, star, end) =>{
    
    let ratingList = array.reduce((acumm, element) => {

        let minutes = parseInt(element.duration.match(/\d+/)[0])

        if(minutes >= star && minutes <= end){
        acumm = [...acumm, element.title]
        }
        return acumm
  ...
  console.log(getDuration(movies, 120, 121))

  /* 6. Obtener una lista de películas con base en su clasificación */
  const getMoviebyClasification =(array, clasificacion) =>{
    let moviesName = array.reduce((acumm, element) => {
      if(element.rating == clasificacion){
        acumm = [...acumm, element.title]        
      }
      return acumm
    },[])
  ...
  console.log(getMoviebyClasification(movies, "PG"))

/*7. Obtener una lista de películas películas estrenadas en un rango de años (por ejemplo, entre 2000 - 2010, los valores de los años deben ser dinámicos)
*/

const getMoviesbyYear = (array, yearStart, yearEnd) => {
 let moviesList = array.reduce((acumm, element)=>{

  if(element.releaseYear >= yearStart && element.releaseYear <= yearEnd)
    acumm =[...acumm, element.title]
    return acumm  
 }, [])
return moviesList
...
console.log(getMoviesbyYear(movies, 2010, 2015))

/*      8. Obtener una lista de películas con base en el país al que pertenecen
 */
const getMoviebyCountry = (array, countryUser) =>{

  let moviesList = array.reduce((acumm, element) => {
    
    if(element.country === countryUser){
      acumm = [...acumm, element.title]
    }
    return acumm 
...
console.log(getMoviebyCountry(movies, "Canadá"))
...
const getaNoOscarMovies = (array) =>{

  let noOscar = array.reduce((acumm, element) =>{

    if(element.oscarAwards == 0){
      acumm = [...acumm, element.title]
    }
